...
export const TypewriterEffect = ({ words = [], className, cursorClassName }) => {
  ...
  return (
    <div className={cn("text-white sm:text-xl md:text-3xl lg:text-5xl font-bold text-center", className)}>
      {renderWords()}
      <motion.span
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
...
export const TypewriterEffectSmooth = ({ words = [], className, cursorClassName }) => {
        ...
        <div key={`word-${idx}`} className="inline-block">
          {word.text.map((char, index) => (
            <motion.span
              ...
              animate={{ opacity: 1 }}
              transition={{
                duration: 0.05 * (index + 1), // Adjust for smoother animation
              }}
  ...
  return (
    <div className={cn("flex space-x-1 my-6", className)}>
      ...
      >
        <div
          className="text-xs sm:text-xl md:text-xl lg:text-2xl xl:text-2xl"
